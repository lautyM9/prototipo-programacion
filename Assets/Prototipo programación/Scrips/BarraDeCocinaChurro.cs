...
public class BarraCocinaChoclo : MonoBehaviour
{
    [Header("Visuales")]
    public Image barraRelleno;             // Imagen tipo Filled
    public GameObject borde;               // Borde de la barra
    public GameObject cartelChocloAgarrado;
    public GameObject cartelChocloEnMaquina;
    ...
    public void EmpezarCocina()
    {
        ...
        // Resetear visuales
        if (cartelChocloAgarrado != null) cartelChocloAgarrado.SetActive(true);
        if (cartelChocloEnMaquina != null) cartelChocloEnMaquina.SetActive(false);
        if (cartelChocloListo != null) cartelChocloListo.SetActive(false);
    ...
    private IEnumerator ProcesoCoccion()
    {
        ...
        if (cartelChocloAgarrado != null) cartelChocloAgarrado.SetActive(false);

        // Paso 2: Mostrar ingrediente en máquina
        if (cartelChocloEnMaquina != null) cartelChocloEnMaquina.SetActive(true);
        ...
        if (cartelChocloEnMaquina != null) cartelChocloEnMaquina.SetActive(false);
        if (cartelChocloListo != null) cartelChocloListo.SetActive(true);
        if (cartelTiempo != null) cartelTiempo.SetActive(false);
        if (borde != null) borde.SetActive(false);
        if (barraRelleno != null) barraRelleno.gameObject.SetActive(false);
